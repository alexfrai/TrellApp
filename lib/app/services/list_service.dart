...
class ListService {
  ...
  static Future<dynamic> createList(String name, String Board_ID) async {
  print("Board ID: ${Board_ID}");
  try {
    final url = Uri.parse(
      "$baseUrl/lists?name=${Uri.encodeComponent(name)}&idBoard=${Uri.encodeComponent(Board_ID)}&key=$API_KEY&token=$API_TOKEN"
    );
